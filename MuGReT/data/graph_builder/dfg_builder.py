...
class DDGNode():
    ...
    def __init__(self, node_key: str = None, node_type: str = None, defs: list = [], uses : list = [], unknow: list = []) -> None:
        if node_key == None:
            log.error('DDGNode initialization need node type, exit.')
        ...
        for _pred in predecessors:
            ...
            if is_statement and _pred not in visited and check_edge(cfg_cpg, _pred, current_node):
                if _pred not in def_use_chain:
                    ...
                    log.warning(f"Parent node {_pred} not in def_use_chain, skipping.")
                    print(f"Parent node {_pred} not in def_use_chain, skipping.")
...
def has_dd_rel(use_var, def_node):
    """Check whether use_var is defined by def_node.
    """

    if use_var == None or def_node == None:
        log.error('Use variable or Def Node is None, Exit.')
        exit(-1)
...
def combine_fields(left_v, right_v):
