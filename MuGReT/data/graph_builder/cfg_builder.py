from PycharmProject.MuGReT-DWA.MuGReT.data.graph_builder.code_graph import EDGE_DICT, Queue
...
def cfg_ifstatement(node, src, tgt, edgetype):
    ...
    if len(children) == 2:
        ...
        if type(children[-1].data).__name__ == 'Statement':
            ...
            # handle single if without else
            then_entrynode, then_frige = get_cfg_edge(children[-1], src, tgt, edgetype)

            # --- 【修改点 1】检查 then_entrynode 是否为空 ---
            if then_entrynode is not None:
                src.append(entry_node.id)
                tgt.append(then_entrynode.id)
                edgetype.append([EDGE_DICT['cfg_edge']])
                fringe_nodes = then_frige + [node]
            else:
                # 如果 then 块为空，只有 if 节点本身作为出口
                fringe_nodes = [node]
        ...
        # --- 【修改点 2】处理 then 分支的安全连接 ---
        if then_entrynode is not None:
            src.append(entry_node.id)
            tgt.append(then_entrynode.id)
            edgetype.append([EDGE_DICT['cfg_edge']])
            then_exits = then_frige
        else:
            # then 块为空时，if 节点本身就是该路径的出口
            then_exits = [entry_node]

        # --- 【修改点 3】处理 else 分支的安全连接 ---
        if else_entrynode is not None:
            src.append(entry_node.id)
            tgt.append(else_entrynode.id)
            edgetype.append([EDGE_DICT['cfg_edge']])
            else_exits = else_frige
        else:
            # else 块为空时，if 节点本身也是该路径的出口
            else_exits = [entry_node]

        # 汇总两个分支的出口节点
        fringe_nodes = then_exits + else_exits

    else:
        log.error("If statement has more than 3 children")
        print("If statement has more than 3 children")
        exit(-1)
